...
@implementation HotelSuppliesProductCell
{
    //是否需要显示特惠
    BOOL isNeedSpecail;
...
-(void)SetUpCustomTitleLabel
{
    ...
    [_customTitleLabel makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.left.mas_equalTo(self.left).with.offset(GET_SCAlE_LENGTH(125));
    }];
...
-(void)cellForRowWithModel:(HSProduct *)model
{
    ...
    NSString * logoString = [model.logo stringByReplacingOccurrencesOfString:@" " withString:@""];
    
    if ([model.price isKindOfClass:[NSString class]]) {
        [self.customHeadImageView sd_setImageWithURL:[NSURL URLWithString:logoString] placeholderImage:[UIImage imageNamed:@"headimage"]];
    }
    ...
    self.customScoreLabel.text = [NSString stringWithFormat:@"评分%.1f分",score];
    if (isStarLevel) {
        self.customStarLevelLabel.hidden = NO;
        self.customDistanceLabel.hidden = NO;
...
//    self.customDistanceLabel.text = [NSString stringWithFormat:@"距离：%@km",model.distance];
    ...
    [_customTitleLabel updateConstraints:^(MASConstraintMaker *make) {
        ...
        {
            make.size.mas_equalTo(CGSizeMake(titleWidth + 2, 14));
        }
    }];
...
-(void)cellForRowWithHotelModel:(Hotels *)model
{
    ...
    CGFloat score = ([model.score floatValue] < 10)?[model.score floatValue]:([model.score floatValue]/10);
    ...
    //    _customDistanceLabel.numberOfLines = 2;
