...
@implementation ProductDetailTabbar
{
    //回调block
...
- (void)loadCustomViewWithFrame:(CGRect)frame
{
    ...
    _GuestclothingButton.backgroundColor = [UIColor greenColor];
    ...
    _telPhoneButton.backgroundColor = [UIColor yellowColor];
...
//    [_collectionButton setTitle:@"收藏" forState:UIControlStateNormal];
    ...
    [_ShoppingCartButton makeConstraints:^(MASConstraintMaker *make) {
        make.left.mas_equalTo(_collectionButton.right);
        make.bottom.mas_equalTo(self.bottom);
        make.size.mas_equalTo(CGSizeMake((((SCREEN_WIDTH - 103 * 3/2)/2)), frame.size.height));
    }];
