...
+ (NSString *)convertToneNumber2ToneMarkWithNSString:(NSString *)pinyinStr {
  ...
  if ([lowerCasePinyinStr matchesPatternRegexPattern:@"[a-z]*[1-5]?"]) {
    ...
    if ([lowerCasePinyinStr matchesPatternRegexPattern:@"[a-z]*[1-5]"]) {
        long int tuneNumber = [PinyinFormatter getNumericValue:[lowerCasePinyinStr characterAtIndex:lowerCasePinyinStr.length -1]];
      long int indexOfA = [lowerCasePinyinStr indexOf:charA];
      long int indexOfE = [lowerCasePinyinStr indexOf:charE];
      long int ouIndex = [lowerCasePinyinStr indexOfString:ouStr];
      if (-1 != indexOfA) {
      ...
      if ((defautlCharValue != unmarkedVowel) && (defautlIndexValue != indexOfUnmarkedVowel)) {
        long int rowIndex = [allUnmarkedVowelStr indexOf:unmarkedVowel];
