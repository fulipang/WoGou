...
<UICollectionViewDataSource,UICollectionViewDelegate>
{
     ...
     *  起始位置
     */
    CalendarDayModel * _calendarDayBegin;
    
    //离店事件
    CalendarDayModel * _calendarDayEnd;
...
- (void)initView{
    ...
    [self setNavigationBarTitle:@"选择入住日期"];
    ...
    self.collectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0, 64, SCREEN_WIDTH, SCREEN_HEIGHT - 64) collectionViewLayout:layout]; //初始化网格视图大小
...
-(void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (model.style == CellDayTypeFutur || model.style == CellDayTypeWeek ||model.style == CellDayTypeClick) {
        ...
        if (_calendarDayBegin == nil) {
            ...
            _calendarDayBegin = model;
            [month_Array setObject:model atIndexedSubscript:indexPath.row];
        }else{
            _calendarDayEnd = model;
            if (self.calendarblock) {
                
                self.calendarblock(_calendarDayBegin,_calendarDayEnd);//传递数组给上级
